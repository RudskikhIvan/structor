  ...
  describe 'as_hashes' do
    ...
    describe 'without options' do
      ...
      it 'returns hashes with all keys' do
        ...
        expect(hashes.map{|h| h['address']}).to all(
          have_keys('id', 'user_id', 'address', 'city', 'approved', 'created_at', 'updated_at'))
      end
    ...
    describe 'with only option' do
      ...
      it 'does not instantiate models' do
        expect(Address).to_not receive(:instantiate)
        expect(User).to_not receive(:instantiate)
        expect(User.as_hashes(include: {address: {only: %i[address city]}}).size).to eq(3)
      end
    ...
    describe 'with except option' do
      ...
      it 'returns hashes without excepted keys' do
        hashes = User.as_hashes(include: {address: {except: %i[city created_at updated_at]}})
        expect(hashes).to all have_keys('id', 'first_name', 'last_name', 'email', 'created_at', 'updated_at', 'address')
        expect(hashes.map{|h| h['address']}).to all match({
           'id' => a_kind_of(Integer),
           'user_id' => a_kind_of(Integer),
    ...
    describe 'with proc option' do
      ...
      it 'does not instantiate models' do
        expect(User).to_not receive(:instantiate)
        expect(Address).to_not receive(:instantiate)
        expect(User.as_hashes(include: {address: {procs: {zip_city: ->(h){ "#{h['city']} #{h['zip_code']}" }}}}).size).to eq(3)
      end

      it 'returns hashes with virtual key' do
        hashes = User.as_hashes(include: {address: {procs: {zip_city: ->(h){ "#{h['city']} #{h['zip_code']}" }}}})
        expect(hashes).to all have_keys('id', 'first_name', 'last_name', 'email', 'created_at', 'updated_at', 'address')
        expect(hashes.map{|h| h['address']}).to all(
          have_keys('id', 'address', 'user_id', 'city', 'zip_code', 'zip_city'))
      end
      ...
      it 'returns hashes with correct values' do
        ...
        User.includes(:address).zip(hashes) do |user, hash|
          expect(user.attributes).to eq(hash.except('address'))
          expect(user.address.attributes).to eq(hash['address'].except('zip_city'))
          expect(hash['address']['zip_city']).to eq("#{user.address.city} #{user.address.zip_code}")
        end
      end
    end
    ...
    describe 'with sql alias' do
      ...
      it 'does not instantiate models' do
        expect(Address).to_not receive(:instantiate)
        expect(User).to_not receive(:instantiate)
        expect(User.as_hashes(include: {address: {only: [:id, "city || ' ' || zip_code as zip_city"]}}).size).to eq(3)
      end

      it 'returns hashes with virtual key' do
        hashes = User.as_hashes(include: {address: {only: [:id, "city || ' ' || zip_code as zip_city"]}})
        expect(hashes).to all have_keys('id', 'first_name', 'last_name', 'email', 'created_at', 'updated_at', 'address')
        expect(hashes.map{|h| h['address']}).to all have_keys('id', 'zip_city')
      end
      ...
      it 'returns hashes with correct values' do
        ...
        User.includes(:address).zip(hashes) do |user, hash|
          expect(user.attributes).to eq(hash.except('address'))
          expect(user.address.attributes.slice('id', 'user_id')).to eq(hash['address'].except('zip_city'))
          expect(hash['address']['zip_city']).to eq("#{user.address.city} #{user.address.zip_code}")
        end
      end
    end
  ...
  describe 'as_structs' do
    ...
    describe 'without options' do
      ...
      it 'returns structs with all keys' do
        ...
        expect(structs.map{|s| s.address.to_h}).to all have_keys(*%i[id user_id address city approved created_at updated_at zip_code])
      end
    ...
    describe 'with except option' do
      ...
      it 'returns structs without excepted keys' do
        structs = User.as_structs(include: {address: {except: %i[approved created_at updated_at city]}})
        expect(structs.map(&:to_h)).to all have_keys(*%i[id first_name last_name email created_at updated_at address])
        expect(structs.map{|s| s.address.to_h}).to all match({
          id: a_kind_of(Integer),
          user_id: a_kind_of(Integer),
      ...
      it 'returns structs with correct values' do
        ...
        User.includes(:address).all.zip(structs) do |user, struct|
          expect(user.attributes.symbolize_keys).to eq(struct.to_h.except(:address))
          expect(user.address.attributes.slice(*%w[id address user_id zip_code city]).symbolize_keys).to eq(struct.address.to_h)
        end
      end
    end
    ...
    describe 'with proc option' do
      ...
      it 'does not instantiate models' do
        expect(User).to_not receive(:instantiate)
        expect(Address).to_not receive(:instantiate)
        expect(User.as_structs(include: {address: {procs: {zip_city: ->(h){ "#{h['city']} #{h['zip_code']}" }}}}).size).to eq(3)
      end

      it 'returns structs with virtual key' do
        structs = User.as_structs(include: {address: {procs: {zip_city: ->(h){ "#{h['city']} #{h['zip_code']}" }}}})
        expect(structs.map(&:to_h)).to all have_keys(*%i[id first_name last_name email created_at updated_at address])
        expect(structs.map{|s| s.address.to_h}).to all have_keys(*%i[id address user_id city zip_code zip_city])
      end
      ...
      it 'returns structs with correct values' do
        ...
        User.includes(:address).zip(structs) do |user, struct|
          expect(user.attributes.symbolize_keys).to eq(struct.to_h.except(:address))
          expect(user.address.attributes.symbolize_keys).to eq(struct.address.to_h.except(:zip_city))
          expect(struct.address.zip_city).to eq("#{user.address.city} #{user.address.zip_code}")
        end
      end
    end
    ...
    describe 'with sql alias' do
      ...
      it 'does not instantiate models' do
        expect(Address).to_not receive(:instantiate)
        expect(User).to_not receive(:instantiate)
        expect(User.as_structs(include: {address: {only: [:id, "city || ' ' || zip_code as zip_city"]}}).size).to eq(3)
      end

      it 'returns structs with virtual key' do
        structs = User.as_structs(include: {address: {only: [:id, "city || ' ' || zip_code as zip_city"]}})
        expect(structs.map(&:to_h)).to all have_keys(*%i[id first_name last_name email created_at updated_at address])
        expect(structs.map{|s| s.address.to_h}).to all have_only_keys(*%i[id zip_city])
      end
      ...
      it 'returns structs with correct values' do
        ...
        User.includes(:address).zip(structs) do |user, struct|
          expect(user.attributes.symbolize_keys).to eq(struct.to_h.except(:address))
          expect(user.address.attributes.slice('id', 'user_id').symbolize_keys).to eq(struct.address.to_h.except(:zip_city))
          expect(struct.address.zip_city).to eq("#{user.address.city} #{user.address.zip_code}")
        end
      end
    end
